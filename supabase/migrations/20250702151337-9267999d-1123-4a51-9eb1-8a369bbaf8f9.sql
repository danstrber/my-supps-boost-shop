-- Fix ambiguous referral_code reference in update_user_post_auth function
CREATE OR REPLACE FUNCTION public.update_user_post_auth(user_auth_id uuid, user_name text DEFAULT NULL::text, user_country text DEFAULT NULL::text, referral_code text DEFAULT NULL::text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
BEGIN
  ...
  IF referral_code IS NOT NULL THEN
    ...
    WHERE u1.referral_code = referral_code  -- This was the ambiguous reference - now clearly refers to the input parameter
